...
static void suble_gpio_irq_cb(uint32_t pin)
{
    tuya_ble_app_evt_send_with_data(APP_EVT_ANTILOST_KEY, &pin, sizeof(uint32_t));
}
...
//                if(bk_gpio_get_input(pin) == LEVEL_LOW)
                {
                    lock_timer_start(LOCK_TIMER_ANTILOCK_KEY);
                }
            } break;
...
#define SUBLE_BUZZER_CLOCK 16000000
    ...
    if(freq < SUBLE_BUZZER_CLOCK/65535) {
        SUBLE_PRINTF("Error: freq not support");
        return;
    }
...
void suble_gpio_buzzer_hold(void)
{
    ...
    suble_timer_start_0(SUBLE_TIMER103, 1000, 1);
}
